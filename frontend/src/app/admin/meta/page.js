...
function ConfiguracoesTab({ connections, onReload }) {
  ...
  async function connect() {
    ...
    try {
      await api.metaConnect(form.ad_account_id, form.access_token);
  ...
  return (
    <div className="space-y-5 max-w-lg">
      ...
      {connections.length > 0 && (
        <div className="card p-5">
          <h3 className="font-semibold text-slate-900 mb-4">Contas conectadas</h3>
          <div className="space-y-3">
            {connections.map(c => (
              <div key={c.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-800">{c.account_name}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => disconnect(c.id, c.account_name)}
      ...
      {/* Adicionar conta */}
      <div className="card p-5">
        <h3 className="font-semibold text-slate-900 mb-1">Conectar conta Meta Ads</h3>
        <p className="text-xs text-slate-400 mb-4">
          Gere um token de acesso em <strong>business.facebook.com</strong> → Configurações → Tokens de acesso do sistema.
        </p>
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1">ID da conta de anúncios</label>
            <input
              className="input"
              value={form.ad_account_id}
              onChange={e => setForm(f => ({ ...f, ad_account_id: e.target.value }))}
              placeholder="123456789 (sem act_)"
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1">Token de acesso</label>
            <input
          ...
          <button onClick={connect} disabled={saving || !form.ad_account_id || !form.access_token} className="btn-primary w-full py-2.5">
            {saving ? 'Conectando...' : 'Conectar e verificar'}
          </button>
        </div>
      </div>
      ...
      <div className="card p-5 bg-slate-50">
        <h3 className="font-semibold text-slate-700 mb-2 text-sm">Webhook para N8N (verificar alertas)</h3>
        <p className="text-xs text-slate-500 mb-3">
          Configure o N8N para chamar este endpoint periodicamente (ex: a cada hora):
        </p>
        <code className="block bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-700 font-mono break-all">
          POST {typeof window !== 'undefined' ? window.location.origin.replace('3000', '3001') : ''}/meta/alerts/check
        </code>
        <p className="text-xs text-slate-400 mt-2">Inclua o header: <code>Authorization: Bearer [seu_token]</code></p>
      </div>
    </div>
  );
}
